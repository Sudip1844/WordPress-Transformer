<?php
/**
 * Template Name: Zoom QR Code Generator
...
<section class="qr-page-hero">
    <div class="container">
        <h1 class="qr-page-title">
            <span class="dynamic-neon-title">Zoom QR Code Generator</span>
        </h1>
        <p class="qr-page-desc">
            Create a QR code that lets participants join your Zoom meeting instantly with a scan.
        </p>
    </div>
</section>
    ...
    <div class="container">
        <div class="generator-wrapper">
            <div class="form-section">
                <form id="zoom-qr-form">
                    <div class="form-group">
                        <label for="zoom-link">Zoom Meeting Link or ID</label>
                        <input type="text" id="zoom-link" class="form-input" placeholder="https://zoom.us/j/123456789 or meeting ID" required>
                    </div>
                    <div class="form-group">
                        <label for="passcode">Meeting Passcode (Optional)</label>
                        <input type="text" id="passcode" class="form-input" placeholder="abc123">
                    </div>
                </form>
            </div>
            ...
            <div class="preview-section">
                <div class="qr-preview" id="qr-preview">
                    <p style="color: #64748b;">Enter Zoom meeting details to generate QR code</p>
                </div>
    ...
    <div class="container">
        <div class="info-grid">
            <div class="info-card">
                <h3>Benefits</h3>
                <ul>
                    <li>One scan to join the meeting</li>
                    <li>Include passcode automatically</li>
                    <li>Perfect for meeting invitations</li>
                    <li>Works with any Zoom link</li>
                </ul>
            </div>
            <div class="info-card">
                <h3>Use Cases</h3>
...
document.addEventListener('DOMContentLoaded', function() {
    const linkInput = document.getElementById('zoom-link');
    ...
    function generateQR() {
        ...
        if (!link) {
            qrPreview.innerHTML = '<p style="color: #64748b;">Enter Zoom meeting details to generate QR code</p>';
        ...
        if (!link.startsWith('http')) {
            link = 'https://zoom.us/j/' + link.replace(/\D/g, '');
        }
        
        if (passcodeInput.value.trim() && !link.includes('pwd=')) {
            link += (link.includes('?') ? '&' : '?') + 'pwd=' + passcodeInput.value.trim();
        }
    ...
    linkInput.addEventListener('input', generateQR);
    passcodeInput.addEventListener('input', generateQR);
